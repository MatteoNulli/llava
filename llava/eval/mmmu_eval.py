...
def eval_model(args):
    ...
    for _, line in tqdm(questions.iterrows(), total=len(questions)):
        ...
        if args.pixtral:
            input_ids, image_tensor, image_sizes, prompt, qs, placeholder = process(
                line, args, tokenizer, processor, None
            )
        else:
            input_ids, image_tensor, image_sizes, prompt, qs, placeholder = process(
                line, args, tokenizer, processor, model.config
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--question_file", type=str, default="./answers/answers.jsonl")
    parser.add_argument(
        "--question_extension",
        type=str,
