...
python -m llava.eval.mmmu_eval \
    ...
    --question_file $PLAYGROUND_DIR/mmmu/validation-00000-of-00001.parquet \
...
python -m llava.eval.model_vqa_nyu-cvbench \
    --model-path $MODEL_DIR \
    --model-base $MODEL_BASE \
    --question-file $PLAYGROUND_DIR/nyu-cvbench/$SPLIT.parquet \
...
python -m llava.eval.convert_nyu-cvbench_for_submission \
    --annotation-file $PLAYGROUND_DIR/nyu-cvbench/$SPLIT.parquet \
